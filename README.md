...
Because the features selected based on permutation importance do not inform how lenders make decisions of approval using these feature, a logistic regression model was trained and cross-validated (5-fold) utilizing the existing training and validation datasets. The regularization parameter, penalty strength("C" (list of [0.0, 0.0001, 0.001, 0.01, 0.1, 1.0])),  was tuned to maximized the model performance. Finally, using the best "C", the same selected features and all the training data, a logistic regression model was trained and further tested using the independent testing dataset. This offers a way of comparing the coefficient-based relative feature importance. Similar to the random forest model, the ROC-AUC and confusion matrix were reported. 
...
Coefficient-based relative feature importance reported from logistic regression model is shown below. It indicates that lenders more likely approve applications if:
...
and more likely reject applications if:
